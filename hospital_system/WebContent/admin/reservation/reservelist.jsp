...
<body>
		...
		<div class="weadmin-body">
				...
				<c:forEach items="${reservelist }" var="item" varStatus="i">
					<tr data-id="item.count">
						...
						<td class="td-manage">
							...
							<div>
								<span title="删除" class="delBtn" data-id="${item.r_id}" data-username="${item.p_name}" >
			...
			<div class="page">
				<div>
					<c:if test="${page!='1'}">
						<a class="prev" href="<%=request.getContextPath() %>/admin/reservation/reservelist?page=${page-1}">&lt;&lt;</a>
					</c:if>
					 <span class="current">${page }</span>
					 <c:if test="${page+1 <=allpage }">
					 	 <a class="num" href="<%=request.getContextPath() %>/admin/reservation/reservelist?page=${page+1}"> ${page+1}</a>
					 </c:if>
					<c:if test="${page+2<allpapge }">
						 <a class="num" href="<%=request.getContextPath() %>/admin/reservation/reservelist?page=${page+2}">${page + 2 }</a>
					</c:if>
					<c:if test="${page+1 <allpage } ">
						<a class="num" href="<%=request.getContextPath() %>/admin/reservation/reservelist?page=${allpage}">${allpage}</a> 
					</c:if>
					<c:if test="${ page+1 <=allpage}">
						<a class="next" href="<%=request.getContextPath() %>/admin/reservation/reservelist?page=${page+1}">&gt;&gt;</a>
					</c:if>
