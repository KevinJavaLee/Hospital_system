...
<body>
		<div class="weadmin-nav">
			<span class="layui-breadcrumb">
				...
				<a href="javascript:;"> <cite>患者列表</cite></a>
			</span>
		...
		<div class="weadmin-body">
				...
				<form class="layui-form layui-col-md12 we-search" action="" method="get">
					...
					<div class="layui-input-inline">
						<select name="userType">
							<option value="all">请选择查找类型</option>
			...
			<div class="weadmin-block">
				...
				<a class="layui-btn" href="<%=request.getContextPath() %>/admin/patient/addpatient">
			...
			<table class="layui-table" id="memberList">
				...
				<tbody>
				<c:forEach items="${patientlist }" var="item" varStatus="i">
					<tr data-id="item.count">
						<td>
							<div class="layui-unselect layui-form-checkbox"  lay-skin="primary" data-id="${item.p_id}">
								<i class="layui-icon">&#xe605;</i>
							</div>
						</td>
						<td>${item.p_id }</td>
						...
						<td>${item.p_age }</td>
						<td class="td-status">
							<span class="layui-btn layui-btn-normal layui-btn-xs"><c:out value="${item.p_sex }" default="未设置"></c:out></span>
						</td>
						<td class="td-manage">
							
							
							<a title="编辑"  href="<%=request.getContextPath() %>/admin/patient/editpatient?id=${item.p_id}">
								<i class="layui-icon layui-icon-edit"></i>
							</a>
							<!-- <a title="删除"  href="<%=request.getContextPath() %>/admin/deluser?id=${item.id}"> -->
							<div>
								<span title="删除" class="delBtn" data-id="${item.p_id}" data-username="${item.p_username}" >
			...
			<div class="page">
				<div>
					<c:if test="${page!='1'}">
						<a class="prev" href="<%=request.getContextPath() %>/admin/patient/patientlist?page=${page-1}">&lt;&lt;</a>
					</c:if>
					 <span class="current">${page }</span>
					 <c:if test="${page+1 <=allpage }">
					 	 <a class="num" href="<%=request.getContextPath() %>/admin/patient/patientlist?page=${page+1}"> ${page+1}</a>
					 </c:if>
					<c:if test="${page+2<allpapge }">
						 <a class="num" href="<%=request.getContextPath() %>/admin/patient/patientlist?page=${page+2}">${page + 2 }</a>
					</c:if>
					<c:if test="${page+1 <allpage } ">
						<a class="num" href="<%=request.getContextPath() %>/admin/patient/patientlist?page=${allpage}">${allpage}</a> 
					</c:if>
					<c:if test="${ page+1 <=allpage}">
						<a class="next" href="<%=request.getContextPath() %>/admin/patient/patientlist?page=${page+1}">&gt;&gt;</a>
					</c:if>
				...
				if (e.target.parentElement.className=="delBtn"){
					...
					var alertId = layer.confirm('是否确认删除id为'+id+"用户名为："+username+"?", {
					  ...
					  area: ['500px', '300px']
			  		  ...
			  		  //按钮【按钮二】的回调
			  		location.href="<%=request.getContextPath() %>/admin/patient/delpatient?id="+id;
			  		});
				}
				...
				var alertId = layer.confirm('是否确认可以批量删除id为'+delIdList, {
					...
					area: ['500px', '300px']
			  		...
			  		layer.close(alertId); 
			  		  ...
			  		  //按钮【按钮二】的回调
						$.ajax({
						url:"<%=request.getContextPath() %>/admin/patient/delpatient",
						method:"post",
				  	data:{
						ids:delIdList
				  	},
