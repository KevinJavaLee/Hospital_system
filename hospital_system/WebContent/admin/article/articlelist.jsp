...
<body>
		...
		<div class="weadmin-body">
			<div class="layui-row">
				<form class="layui-form layui-col-md12 we-search">
					...
					<div class="layui-inline">
						<input type="text" name="likeuser" placeholder="请输入要查找的内容" autocomplete="off" class="layui-input" />
					</div>
			...
			<div class="weadmin-block">
				...
				<a class="layui-btn" href="<%=request.getContextPath() %>/admin/article/addarticle">
			...
			<table class="layui-table" id="memberList">
				...
				<c:forEach items="${articlelist}" var="item" varStatus="i">
					<tr data-id="item.count">
						<td>
							<div class="layui-unselect layui-form-checkbox"  lay-skin="primary" data-id="${item.a_id}">
								<i class="layui-icon">&#xe605;</i>
							</div>
						</td>
						<td>${item.a_id }</td>
						<td>${item.a_title }</td>
						<td>${item.a_author }</td>
						<td>${item.a_readnum }</td>
						<td>${item.categoryname}</td>
						<td class="td-status">
							<span class="layui-btn layui-btn-normal layui-btn-xs"><c:out value="${item.a_pubtime }" default="未设置"></c:out></span>
						</td>
						<td class="td-manage">
							
							
							<a title="编辑"  href="<%=request.getContextPath() %>/admin/article/editarticle?id=${item.a_id}">
								<i class="layui-icon layui-icon-edit"></i>
							</a>
							<!-- <a title="删除"  href="<%=request.getContextPath() %>/admin/deluser?id=${item.id}"> -->
							<div>
								<span title="删除" class="delBtn" data-id="${item.a_id}" data-title="${item.a_title}" >
			...
			<div class="page">
				<div>
					<c:if test="${page!='1'}">
						<a class="prev" href="<%=request.getContextPath() %>/admin/article/articlelist?page=${page-1}">&lt;&lt;</a>
					</c:if>
					 <span class="current">${page }</span>
					 <c:if test="${(page+1)<=allpage }">
					 	 <a class="num" href="<%=request.getContextPath() %>/admin/article/articlelist?page=${page+1}"> ${page+1}</a>
					 </c:if>
					<c:if test="${(page+2)<allpage }">
						 <a class="num" href="<%=request.getContextPath() %>/admin/article/articlelist?page=${page+2}">${page + 2 }</a>
					</c:if>
					 <c:if test="${(page+1)<allpage }">
						<a class="num" href="<%=request.getContextPath() %>/admin/article/articlelist?page=${allpage}">${allpage}</a> 
					</c:if>
					<c:if test="${ (page+1) <=allpage}">
						<a class="next" href="<%=request.getContextPath() %>/admin/article/articlelist?page=${page+1}">&gt;&gt;</a>
					</c:if>
			...
			tbody.addEventListener("click",function(e){
				...
				if (e.target.parentElement.className=="delBtn"){
					console.log(id);
					console.log(title);
					var alertId = layer.confirm('是否确认删除id为'+id+"标题名为："+title+"?", {
					  ...
					  area: ['500px', '300px']
			  		  ...
			  		  //按钮【按钮二】的回调
			  		location.href="<%=request.getContextPath() %>/admin/article/delarticle?id="+id;
			  		});
				}
				...
				var alertId = layer.confirm('是否确认可以批量删除id为'+delIdList, {
					...
					area: ['500px', '300px']
			  		...
			  		layer.close(alertId); 
			  		  ...
			  		  //按钮【按钮二】的回调
						$.ajax({
						url:"<%=request.getContextPath() %>/admin/article/delarticle",
						method:"post",
				  	data:{
						ids:delIdList
				  	},
