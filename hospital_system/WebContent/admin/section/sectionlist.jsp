...
<body>
		<div class="weadmin-nav">
			<span class="layui-breadcrumb">
				<a href="javascript:;">首页</a> <a href="javascript:;">科室管理</a>
				<a href="javascript:;"> <cite>科室列表</cite></a>
			</span>
		...
		<div class="weadmin-body">
			<div class="layui-row">
				<form class="layui-form layui-col-md12 we-search" action="" method="get" >
					...
					<div class="layui-inline">
						<input type="text" name="likeuser" placeholder="请输入科室名" autocomplete="off" class="layui-input" />
					</div>
			...
			<div class="weadmin-block">
				...
				<a class="layui-btn" href="<%=request.getContextPath() %>/admin/section/addsection">
			...
			<table class="layui-table" id="memberList">
				...
				<tbody>
				<c:forEach items="${sectionlist }" var="item" varStatus="i">
					<tr data-id="item.count">
						...
						<td class="td-status">
							<span class="layui-btn layui-btn-normal layui-btn-xs"><c:out value="${item.s_type }" default="未设置"></c:out></span>
						</td>
						<td class="td-manage">
							
							
							<a title="编辑"  href="<%=request.getContextPath() %>/admin/section/editsection?id=${item.s_id}">
								<i class="layui-icon layui-icon-edit"></i>
							</a>
							<!-- <a title="删除"  href="<%=request.getContextPath() %>/admin/deluser?id=${item.id}"> -->
							<div>
								<span title="删除" class="delBtn" data-id="${item.s_id}" data-name="${item.s_name}" >
			...
			<div class="page">
				<div>
					<c:if test="${page!='1'}">
						<a class="prev" href="<%=request.getContextPath() %>/admin/section/sectionlist?page=${page-1}">&lt;&lt;</a>
					</c:if>
					 <span class="current">${page }</span>
					 <c:if test="${page+1 <=allpage }">
					 	 <a class="num" href="<%=request.getContextPath() %>/admin/section/sectionlist?page=${page+1}"> ${page+1}</a>
					 </c:if>
					<c:if test="${page+2<allpapge }">
						 <a class="num" href="<%=request.getContextPath() %>/admin/section/sectionlist?page=${page+2}">${page + 2 }</a>
					</c:if>
					<c:if test="${page+1 <allpage } ">
						<a class="num" href="<%=request.getContextPath() %>/admin/section/sectionlist?page=${allpage}">${allpage}</a> 
					</c:if>
					<c:if test="${ page+1 <=allpage}">
						<a class="next" href="<%=request.getContextPath() %>/admin/section/sectionlist?page=${page+1}">&gt;&gt;</a>
					</c:if>
			...
			tbody.addEventListener("click",function(e){
				...
				if (e.target.parentElement.className=="delBtn"){
					console.log(id);
					console.log(name);
					var alertId = layer.confirm('是否确认删除id为'+id+"科室名为："+name+"?", {
					  ...
					  area: ['500px', '300px']
			  		  ...
			  		  //按钮【按钮二】的回调
			  		location.href="<%=request.getContextPath() %>/admin/section/delsection?id="+id;
			  		});
				}
				...
				var alertId = layer.confirm('是否确认可以批量删除id为'+delIdList, {
					...
					area: ['500px', '300px']
			  		...
			  		layer.close(alertId); 
			  		  ...
			  		  //按钮【按钮二】的回调
						$.ajax({
						url:"<%=request.getContextPath() %>/admin/section/delsection",
						method:"post",
				  	data:{
						ids:delIdList
				  	},
